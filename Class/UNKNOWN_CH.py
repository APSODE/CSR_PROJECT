...
class PLAYER: #PLAYER에 대한 각종 정보를 리턴 // 현재는 캐릭터의 좌표를 리턴할 예정
    ...
    def PLAYER_INFO(IMG_DIR):
        ...
        PLAYER_IMG = pi.load(CHAR_IMG_DIR)
        ...
        with open(f"{CONFIG_DIR}\\character_config.json", "w", encoding = "utf-8") as WRITE_CHARACTER_PROFILE:
            READ_CHARACTER_DATA["PLAYER_WIDTH"] = PLAYER_WIDTH
            READ_CHARACTER_DATA["PLAYER_HEIGHT"] = PLAYER_HEIGHT

            json.dump(READ_CHARACTER_DATA, WRITE_CHARACTER_PROFILE, indent = 4)            
